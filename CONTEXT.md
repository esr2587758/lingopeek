...
- **History**: The automatic, full record of every query the user has run. History is not curated by the user and its entries expire over time. It is distinct from `收藏` (a curated library) and from `保存` (a per-record retention state).
