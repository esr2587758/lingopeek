...
Lingobar 的语言功能必须先完成设置门禁才能使用。门禁包含 AI 设置和 macOS Accessibility 权限。

没有配置 token 或没有授权 Accessibility 时，触发 Lingobar 不进入正常选区模式或输入模式，而是显示设置引导，要求用户补齐 token、base URL、model 和 Accessibility 权限。

设置完成后，用户再次触发 Lingobar，才进入有选区或无选区的正常工作流。
...
- `语音` 不进入 MVP；麦克风图标可以保留为未来能力占位，但点击时只显示 `暂未启用` 或保持禁用。
...
- `翻译`：显示中文自然翻译、关键表达和语感说明。
- `语法`：只支持英文内容；中文或混合内容时按钮置灰。结果显示主干、修饰、逻辑和可收藏句型。
- `改写`：显示一个主版本，并提供口语、正式、简洁、地道等变体。
- `例句`：显示 3-5 个可复用英文例句。
- `发音`：MVP 不做录音评分；先做系统朗读 / 播放，并展示重音、连读和易错点。

面板行为：

- 点击不同动作时，内容区切换，不关闭面板。
- 点击外部不关闭。
- 结果生成中显示 loading。MVP 不做流式输出，完整 AI 响应返回后一次性渲染结果。
- AI 结果按结构化 JSON 解析后渲染；解析失败时显示错误状态，并允许重试。
- AI 请求失败、token 无效、JSON 解析失败时显示错误面板，提供 `重试` 和 `打开 AI 设置`。
- 错误状态不降级展示本地假结果。
...
- `收藏` 收藏当前面板的默认收藏项，而不是总是收藏原始选区。
- 本地收藏第一版只做保存和最近收藏展示，不做搜索、标签或多端同步。
- `复制` 复制当前主结果。
- 第一版不做直接插入当前 App。
